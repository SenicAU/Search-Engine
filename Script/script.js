...
if ((new URLSearchParams(window.location.search).get("q") || "").startsWith("!")) {
	...
	_$.getHTML(`https://api.simer00.repl.co/www.google.com/search?q=${escape((new URLSearchParams(window.location.search)).get("q").replace(/^!/, ""))}&btnI=Im+Feeling+Lucky`, () => { }).then(html => {
...
function run() {
	...
	document.querySelector(".switch-field").onclick = () => {
		updateHistory();
		ftch(`https://altapi.simer00.repl.co/google?q=${escape(inp.value)}`)
	}
	...
	inp.addEventListener("keyup", (e) => {
		...
		if ((e.key === "Enter" && e.shiftKey)) {
			...
			_$.getHTML(`https://api.simer00.repl.co/www.google.com/search?q=${escape(inp.value.replace(/^!/, ""))}&btnI=Im+Feeling+Lucky`, () => { }).then(html => {
	...
	function ftch(url) {
		(async () => {
			...
			if (mode() == "images") {
				...
				width = 0;
				if (cache[`https://altapi.simer00.repl.co/image-search?q=${escape(inp.value)}`.toLowerCase()]) {
					out.innerHTML = cache[`https://altapi.simer00.repl.co/image-search?q=${escape(inp.value)}`.toLowerCase()].map(i => {
				...
				fetch(`https://altapi.simer00.repl.co/image-search?q=${escape(inp.value)}`).then(res => res.json()).then(json => {
					cache[`https://altapi.simer00.repl.co/image-search?q=${escape(inp.value)}`.toLowerCase()] = json;
			...
			if (mode() === "news") {
				...
				let newsUrl = `https://altapi.simer00.repl.co/news-search?q=${escape(inp.value.trim())}`.toLowerCase();
			...
			out.innerHTML = ""
			let wolframUrl = `https://altapi.simer00.repl.co/quick-answer?q=${escape(inp.value.trim().toLowerCase())}`;
			...
			if (inp.value.toLowerCase().startsWith("define ")) {
				if (!cache[`https://altapi.simer00.repl.co/dictionary?q=${escape(input.value.replace("define ", "").trim())}`.toLowerCase()]) {
					fetch(`https://altapi.simer00.repl.co/dictionary?q=${escape(input.value.replace("define ", "").trim())}`).then(res => res.json()).then(json => {
					...
					let json = cache[`https://altapi.simer00.repl.co/dictionary?q=${escape(input.value.replace("define ", "").trim())}`.toLowerCase()]
					out.innerHTML += `<li class="definition"><h3>${_$.escapeHTML(json.word)}</h3><br><span id="def">${_$.escapeHTML(json.pronounciation)} • ${_$.escapeHTML(json.type.toLowerCase().replace(/^./, (i) => i.toUpperCase()))}</span><br><br><span id="text">${_$.escapeHTML(json.meaning)}</span><br><audio src="${json.audio[0].url}" controls><i>Your browser does not support audio!</i></audio>`
				}
			}
				...
				_$.each(document.querySelectorAll("ul#results > li"), (li) => {
					...
					if (!(non_listen.includes(li.classList[0]))) {
						...
						li.querySelector("#info_icon").onmouseup = (e) => {
							...
							e.stopPropagation();
							fetch(`https://altapi.simer00.repl.co/link-preview?q=${escape(li.querySelector("a").href)}`).then(res => res.json()).then(json => {
								alert({
	...
	async function didYouMean(text) {
		let dym_html = await _$.getHTML(`https://api.simer00.repl.co/google.com/search?q=${escape(text)}`, () => { });
